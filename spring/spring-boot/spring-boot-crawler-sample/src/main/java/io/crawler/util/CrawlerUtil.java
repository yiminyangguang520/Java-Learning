...
public class CrawlerUtil {
    ...
    public static List<Picture> getArticleInfo(String url, BeautifulPictures beautifulPictures, String localPath ) {
        try {
            Connection connect = Jsoup.connect(url);
            Document document;
            document = connect.get();
            Element article = document.getElementById("mkPic");
            Elements imgElement = article.getElementsByTag("img");
            ...
            if (imgElement != null && imgElement.size() > 0) {
                for (Element e : imgElement) {
                    ...
                    // 下载img标签里面的图片到本地
                    CrawlerUtil.saveToFile(url2, localPath);
