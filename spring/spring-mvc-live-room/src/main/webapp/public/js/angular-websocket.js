(function (global, factory) {
  ...
  function $WebSocketProvider($rootScope, $q, $timeout, $websocketBackend) {
    ...
    function $WebSocket(url, protocols, options) {
      ...
      this.reconnectIfNotNormalClose = options
          && options.reconnectIfNotNormalClose || false;
    ...
    $WebSocket.prototype._connect = function _connect(force) {
      if (force || !this.socket || this.socket.readyState
          !== this._readyStateConstants.OPEN) {
        this.socket = $websocketBackend.create(this.url, this.protocols);
        this.socket.onmessage = _angular2.default.bind(this,
            this._onMessageHandler);
        this.socket.onopen = _angular2.default.bind(this, this._onOpenHandler);
        this.socket.onerror = _angular2.default.bind(this,
            this._onErrorHandler);
        this.socket.onclose = _angular2.default.bind(this,
    ...
    $WebSocket.prototype.fireQueue = function fireQueue() {
      while (this.sendQueue.length && this.socket.readyState
      === this._readyStateConstants.OPEN) {
        var data = this.sendQueue.shift();

        this.socket.send(
            isString(data.message) || this.binaryType != 'blob' ? data.message
    ...
    $WebSocket.prototype.onMessage = function onMessage(callback, options) {
      ...
      if (options && isDefined(options.filter) && !isString(options.filter)
    ...
    $WebSocket.prototype._onCloseHandler = function _onCloseHandler(event) {
      ...
      if (this.reconnectIfNotNormalClose && event.code !== this._normalCloseCode
          || this._reconnectableStatusCodes.indexOf(event.code) > -1) {
        this.reconnect();
      }
    };
      ...
      for (var i = 0; i < self.onMessageCallbacks.length; i++) {
        ...
        if (pattern) {
          if (isString(pattern) && message.data === pattern) {
            applyAsyncOrDigest(currentCallback.fn, currentCallback.autoApply,
                message);
    ...
    $WebSocket.prototype._setInternalState = function _setInternalState(state) {
      if (Math.floor(state) !== state || state < 0 || state > 4) {
        throw new Error('state must be an integer between 0 and 4, got: '
            + state);
      }
      ...
      forEach(this.sendQueue, function (pending) {
        pending.deferred.reject(
            'Message cancelled due to closed socket connection');
      });
    };
  ...
  function $WebSocketBackendProvider($log) {
    ...
    this.createWebSocketBackend = function createWebSocketBackend(url,
        protocols) {
  ...
  _angular2.default.module('ngWebSocket', []).factory('$websocket',
      ['$rootScope', '$q', '$timeout', '$websocketBackend',
        $WebSocketProvider]).factory('WebSocket',
      ['$rootScope', '$q', '$timeout', 'WebsocketBackend',
        $WebSocketProvider]).service('$websocketBackend',
      ['$log', $WebSocketBackendProvider]).service('WebSocketBackend',
