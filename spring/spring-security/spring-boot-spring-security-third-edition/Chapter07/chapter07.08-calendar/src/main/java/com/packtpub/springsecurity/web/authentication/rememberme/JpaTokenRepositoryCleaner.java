...
/**
 * <p>
 * A {@link Runnable} which can be used to clean expired persistent remember me tokens generated from {@link JdbcTokenRepositoryImpl}. Note that only a single cleanup is
 * done in {@link #run()} so consumers must loop over invoking the {@link #run()} method in order to perform multiple cleanups.
 * </p>
 * <p>
 * See the configuration file at <code>src/main/webapp/WEB-INF/spring/cleaner.xml</code> to learn how to hook in with <a
 * href="https://docs.spring.io/spring/docs/current/spring-framework-reference/html/scheduling.html">Spring's scheduling abstraction</a>.
 * </p>
 *
 * @author Mick Knutson
 */
public class JpaTokenRepositoryCleaner implements Runnable {
  ...
  /**
   * @param rememberMeTokenRepository the {@link RememberMeTokenRepository} used to perform the cleanup. Cannot be null.
   * @param tokenValidityInMs         used to calculate when a token is expired. If the {@link #run()} method is invoked, tokens older than this amount of time will be
   *                                  deleted. Cannot be less than 1.
   */
