...
/**
 * A Spring Security {@link AuthenticationProvider} that uses our {@link CalendarService} for authentication. Compare this to our {@link CalendarUserDetailsService} which
 * is called by Spring Security's {@link DaoAuthenticationProvider}.
 *
 * <p>
 * This implementation has updates that allow for an additional parameter named domain to be passed in. In a real world application the service tier would likely require
 * three distinct fields (username, password, and domain) but since we are learning about Spring Security not services we reuse our existing service by concatenating the
