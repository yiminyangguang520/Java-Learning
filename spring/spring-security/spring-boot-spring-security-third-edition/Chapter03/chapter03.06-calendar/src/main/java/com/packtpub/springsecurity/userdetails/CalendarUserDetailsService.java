...
/**
 * Integrates with Spring Security using our existing {@link CalendarUserDao} by looking up a {@link CalendarUser} and converting it into a {@link UserDetails} so that
 * Spring Security can do the username/password comparison for us.
 *
 * @author Rob Winch
 */
@Component
public class CalendarUserDetailsService implements UserDetailsService {
  ...
  private static final Logger logger = LoggerFactory.getLogger(CalendarUserDetailsService.class);
  ...
  /**
   * Lookup a {@link CalendarUser} by the username representing the email address. Then, convert the {@link CalendarUser} into a {@link UserDetails} to conform to the
  ...
  /**
   * There are advantages to creating a class that extends {@link CalendarUser}, our domain notion of a user, and implements {@link UserDetails}, Spring Security's notion
   * of a user.
   * <ul>
   ...
   * Spring Security's <a href="http://static.springsource.org/spring-security/site/docs/3.1.x/reference/remember-me.html">Remember-Me Authentication</a></li>
   * </ul>
   *
   * @author Rob Winch
   */
