...
/**
 * <p>
 * A {@link PersistentTokenRepository} that looks up and stores the {@link PersistentRememberMeToken} based upon the seriesId + the current user's IP Address. This means
 * that in order to use the token, the user must have the same IP address.
 * </p>
 * <p>
 * The implementation relies on Spring's {@link RequestContextHolder} to populated. In our application, we choose to enable this by adding the in our WebMvcConfig.java as
 * shown below. The dependency on spring-web is not an issue since spring-security-web already depends on spring-web and we are implementing an interface in
 * spring-security-web.
 * </p>
 *
 * <p>
...
public final class IpAwarePersistentTokenRepository implements PersistentTokenRepository {
  ...
  private static final Logger logger = LoggerFactory.getLogger(IpAwarePersistentTokenRepository.class);

  private final PersistentTokenRepository delegateRepository;

  /**
   * Creates a new {@link IpAwarePersistentTokenRepository} that after converting the seriesId to contain the current user's IP address will use the delegateRepository to
   * do all the work.
   */
  ...
  /**
   * Stores the new {@link PersistentRememberMeToken} but first changes the series to be series + currentIpAddress and then delegates to the injected {@link
   * PersistentTokenRepository} to do all the work.
   */
  @Override
  ...
  /**
   * Updates the token with the id series + currentIpAddress and then delegates to the injected {@link PersistentTokenRepository} to do all the work.
   */
  @Override
  ...
  /**
   * Gets the {@link PersistentRememberMeToken} for the given seriesId + currentIpAddress. By always adding the IP address to the identifier we guarantee that the token
   * can only be retrieved if the IP of the original token matches the current user's token. It then delegates to the injected {@link PersistentTokenRepository} to do all
   * the work.
   */
  @Override
