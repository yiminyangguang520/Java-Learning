...
/**
 * Created by Yuicon on 2017/7/16. https://github.com/Yuicon
 */
@Service()
...
public class EntryServiceImpl implements EntryService {
  ...
  public JsonResult<Entry> findOne(String id) {
    return JsonResult.<Entry>builder().data(entryRepository.findById(id).get()).build();
  }
  ...
  private void addBrowseLog(HttpServletRequest request) {
    ...
    if (authHeader != null && authHeader.startsWith(tokenHead)) {
      ...
      User user = userRepository.findByAccount(jwtTokenUtil.getUsernameFromToken(authToken));
      if (user != null) {
        browseLog = Optional.ofNullable(browseLogRepository.findByUid(user.getId()));
      ...
      browseLog = Optional.ofNullable(browseLogRepository.findByIp(request.getRemoteAddr()));
      if (browseLog.isPresent()) {
        BrowseLog browseLog1 = browseLog.get();
        browseLog1.setCount(browseLog1.getCount() + 1);
        browseLog1.setLastTime(new Date(System.currentTimeMillis()));
